...
const actions = {
    ...
    // 保存用户分享海报信息
    saveUserShareInfo ({ commit, state }, param) {
        let promise = new Promise(function (resolve, reject) {
            let url = api.SAVE_USER_SHARE_PLACARD_INFO
    ...
    // 获取用户分享海报信息
    getUserShareInfo ({ commit, state }, shareId) {
        let promise = new Promise((resolve, reject) => {
            let url = api.GET_USER_SHALE_INFO.replace('{0}',shareId)
            axios.get(url).then((response) => {
                if (response.data.code == 0) {
                    let data = response.data.data;
                    resolve(data)
